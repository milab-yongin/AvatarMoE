...
class HierarchicalPoseEncoder(nn.Module):
    ...
    Based on LEAP (Mihajlovic et al., CVPR 2021) and adapted from
    3DGS-Avatar (Qian et al., CVPR 2024).

    Modified for AvatarMoE to:
        - output both joint-level and global pose features,
        - support GMM-based gating,
        - incorporate SMPL shape parameters (betas),
        - use a larger MLP for improved expressivity.
    """
    def __init__(self, num_joints=24, rel_joints=False, dim_per_joint=16, global_dim=64, out_dim=-1, **kwargs):
        super().__init__()
        ...
        full_pose_dim = 9 * self.num_joints + 3 * self.num_joints + 10
        self.layer_0_external = nn.Linear(full_pose_dim, self.global_dim)
        self.layer_0_internal = nn.Linear(full_pose_dim, dim_per_joint)

        dim_feat_per_joint = 9 + 3 + 1 + dim_per_joint

        layers = []
        for idx in range(num_joints):
            layer = nn.Sequential(
                nn.Linear(dim_feat_per_joint, dim_feat_per_joint * 2),
                nn.ReLU(),
                nn.Linear(dim_feat_per_joint * 2, dim_per_joint)
            )
    ...
    def forward(self, rots, Jtrs, shape_params, skinning_weight=None):
        B = rots.size(0)
        if self.rel_joints:
            with torch.no_grad():
                Jtrs_rel = Jtrs.clone()
        ...
        rots_flat = rots.view(B, self.num_joints, -1)
        jtrs_flat = Jtrs_proc.view(B, self.num_joints, -1)

        full_pose_feat = torch.cat([rots.view(B, -1), Jtrs.view(B, -1)], dim=-1)
        
        if shape_params is not None:   
            global_in = torch.cat([full_pose_feat, shape_params], dim=-1)
        else:
            global_in = full_pose_feat

        external_global_feat = self.layer_0_external(global_in)
        internal_global_feat = self.layer_0_internal(global_in)
        ...
        for j_idx in range(self.num_joints):
            ...
            if parent == -1:
                ...
                in_feat = torch.cat([rot, Jtr.view(B, -1), bone_l, internal_global_feat], dim=-1)
            else:
                parent_feat = out[parent]
                bone_l = torch.norm(Jtrs_proc[:, j_idx, :] - Jtrs_proc[:, parent, :], dim=-1, keepdim=True)
                in_feat = torch.cat([rot, Jtr.view(B, -1), bone_l, parent_feat], dim=-1)

            out[j_idx] = self.layers[j_idx](in_feat)

        final_latents = torch.cat(out, dim=-1)
        final_latents = self.out_layer(final_latents)

        return final_latents, external_global_feat
