...
class Scene:
    ...
    def __init__(self, cfg, gaussians : GaussianModel, save_dir : str):
        ...
        self.gaussians.create_from_pcd(self.test_dataset.readPointCloud(), metadata=self.metadata, spatial_lr_scale=self.cameras_extent)
    ...
    def get_boundary_consistency_loss(self):
        loss_reg = self.converter.deformer.non_rigid.regularization()
        loss_boundary = loss_reg.get('loss_boundary', torch.tensor(0.).cuda())
        return loss_boundary

    def get_coherence_loss(self):
        loss_reg = self.converter.deformer.non_rigid.regularization()
        loss_coherence = loss_reg.get('loss_coherence', torch.tensor(0.).cuda())
        return loss_coherence

    def get_gating_loss(self):
        loss_reg = self.converter.deformer.non_rigid.regularization()
        loss_gating = loss_reg.get('loss_gating', torch.tensor(0.).cuda())
        return loss_gating
    
    def get_balance_loss(self):
        loss_reg = self.converter.deformer.non_rigid.regularization()
        loss_balance = loss_reg.get('loss_balance', torch.tensor(0.).cuda())
        return loss_balance    
