...
from utils.visualize import save_render_comparison
...
def training(config):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # expert interval coherence loss
        lambda_coherence = C(iteration, config.opt.lambda_coherence)
        if lambda_coherence > 0:
            loss_coherence = scene.get_coherence_loss()
            loss += lambda_coherence * loss_coherence
        else:
            loss_coherence = torch.tensor(0.).cuda()    
        ...
        if iteration % 50 == 0:
            save_render_comparison(
                ...
                ssim=0.0,
                lpips=0.0,
                save_dir=os.path.join(config.exp_dir, "train_comparisons")
            )
        ...
        with torch.no_grad():
            ...
            log_loss = {
                ...
                'loss/coherence_loss': loss_coherence.item(),
            ...
            _ = validation(
                ...
                (pipe, background), config.exp_dir)
...
def validation(iteration, testing_iterations, testing_interval, scene : Scene, evaluator, renderArgs, exp_dir):
    ...
    with torch.no_grad():
        try:
            coherence_loss_val = scene.get_coherence_loss().item()
        except Exception:
    ...
    for config in validation_configs:
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, data_idx in enumerate(config['cameras']):
                ...
                metrics_test = evaluator(image, gt_image, opacity=opacity_image, gt_mask=gt_mask)
                ...
                save_render_comparison(
                    ...
                    rendered_image=image,
                    iteration=iteration,
                    psnr=metrics_test["psnr"],
                    ssim=metrics_test["ssim"],
                    lpips=metrics_test["lpips"],
                    save_dir=os.path.join(exp_dir, "comparisons")
                )
            ...
            print(f"  >> Coherence: {coherence_loss_val:.6f}, L1: {l1_test.item():.6f}, LPIPS: {lpips_test.item():.6f}, PSNR: {psnr_test.item():.6f}, SSIM: {ssim_test.item():.6f}")

            wandb.log({
                config['name'] + '/loss_viewpoint - coherence': coherence_loss_val,
