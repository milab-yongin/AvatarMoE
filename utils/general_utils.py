...
class Evaluator(nn.Module):
    ...
    def forward(self, inputs, targets, **kwargs):
        ...
        self.fid.update(inputs, targets)
...
class FID(nn.Module):
    def __init__(self, feature=2048, device="cuda"):
        super().__init__()
        self.fid_metric = FrechetInceptionDistance(feature=feature, normalize=False).to(device)
    ...
    def update(self, inputs, targets):
        ...
        inputs_uint8 = (inputs.clamp(0, 1) * 255).to(torch.uint8)
        targets_uint8 = (targets.clamp(0, 1) * 255).to(torch.uint8)

        self.fid_metric.update(targets_uint8, real=True)
        self.fid_metric.update(inputs_uint8, real=False)
...
class PSEvaluator(nn.Module):
    ...
    def forward(self, rgb, rgb_gt, **kwargs):
        # torchmetrics assumes NCHW format
        self.fid.update(rgb, rgb_gt)
        rgb = rgb.unsqueeze(0)
